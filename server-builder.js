...
const { updateDependencyLines } = require('./utils.js');
...
class ServerBuilder {
  constructor (args = {}) {
    ...
    this.serverVersions = _.reduce(versionConfiguration, (acc, value, key) => {
      if (VERSION_KEYS.includes(key)) {
        ...
        console.log(`Got unexpected '${key}' in toolsVersion block of the build configuration`);
      }
      return acc;
    }, {});
  ...
  async insertAdditionalDependencies () {
    ...
    for (const propName of DEPENDENCY_PROP_NAMES) {
      ...
      console.log(`Adding the following ${propName} to build.gradle.kts: ${deps}`);
  ...
  async runBuildProcess () {
    ...
    console.log(`Beginning build with command '${cmd} ${args.join(' ')}' ` +
    ...
    console.log(`${logMsg}. To change this, use 'showGradleLog' desired capability`);
...
module.exports.ServerBuilder = ServerBuilder;
